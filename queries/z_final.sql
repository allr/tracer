...
CREATE VIEW IF NOT EXISTS running_time_details_pct_new AS
...
dologic2_self + dorelop_self ) / CAST(TotalRuntime AS REAL), 4) AS
Arith,
  ROUND(100 * (builtinsum_self + specialsum_self + dotSpecial2_self) /
CAST(TotalRuntime AS REAL), 4) AS BuiltIn_Special,
  ROUND(100 * (startup_self+install_self) / CAST(TotalRuntime AS REAL), 4)
AS Other
...
CREATE VIEW IF NOT EXISTS memory_used_vs_alloc AS
SELECT
    name,
...
CREATE VIEW IF NOT EXISTS memory_used_pct_new AS
...
CREATE VIEW runtime_average AS
  SELECT
    ...
    ROUND( (startup_self+install_self), 4) AS Other,
    totalruntime
  FROM time_summary
  left join traces
  where traces.id=time_summary.id
...
CREATE VIEW relative_vector_alloc_counts AS
  select
    name,
    round(100 * allocatedonevectors_tl / cast((allocatedsmallvectors_tl + allocatedonevectors_tl + allocatedlargevectors_tl) as real), 2),
    round(100 * allocatedsmallvectors_tl / cast((allocatedsmallvectors_tl + allocatedonevectors_tl + allocatedlargevectors_tl) as real), 2),
    round(100 * allocatedlargevectors_tl / cast((allocatedsmallvectors_tl + allocatedonevectors_tl + allocatedlargevectors_tl) as real), 2)
...
CREATE VIEW relative_vector_alloc_counts_overall AS
  select
    name,
